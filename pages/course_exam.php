...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_exam') {
    ...
    if ($passed) {
        ...
        if (!$badge) {
            // Create badge
            $pdo->prepare("INSERT INTO badges (name, description, icon, requirements) VALUES (?, ?, ?, ?)")
                ->execute([$badge_name, "Lulus ujian " . $exam['title'], "🏆", "Lulus ujian dengan KKM " . $exam['min_score_passing']]);
