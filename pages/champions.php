...
$champion = $top10[0] ?? null;

// Siswa yang paling banyak lulus ujian (Master Badges)
$stmt_master = $pdo->query("SELECT u.id, u.name, u.picture, u.profile_color, COUNT(ub.id) as total_master
                     FROM user_badges ub
                     ...
                     LIMIT 8");
$masters = $stmt_master->fetchAll();
?>

<div class="dash-left" style="grid-column: 1 / -1; max-width: 1000px; margin: 0 auto; width: 100%;">

    <div style="text-align:center; margin-bottom: 2.5rem;">
        <h1 style="font-size: 2.2rem; color: var(--dash-text); margin-bottom: 0.5rem; display:flex; justify-content:center; align-items:center; gap:12px;">
        ...
        <p style="color: var(--dash-text-muted); font-size: 1.05rem;">Apresiasi untuk siswa dengan pencapaian XP, badge, dan ujian terbaik.</p>
    </div>
    ...
    <div style="background: linear-gradient(135deg, #f59e0b 0%, #ef4444 100%); border-radius: 24px; padding: 2.5rem; margin-bottom: 2.5rem; display:flex; align-items:center; gap: 2rem; color: white; position: relative; overflow: hidden;">
        ...
        <div style="width: 90px; height: 90px; border-radius: 50%; background: rgba(255,255,255,0.25); backdrop-filter: blur(10px); display:flex; align-items:center; justify-content:center; font-weight:800; font-size: 2rem; flex-shrink:0; border: 3px solid rgba(255,255,255,0.5); position:relative; z-index:1;">
            <?php if (!empty($champion['picture'])): ?>
                <img src="<?php echo htmlspecialchars($champion['picture']); ?>" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">
            <?php else: ?>
                <?php echo substr(htmlspecialchars($champion['name']), 0, 1); ?>
            <?php endif; ?>
        </div>
